...
class MLflowTracker:
    ...
    def _configurar(self) -> None:
        ...
        artifact_location = str(self.root_dir / 'mlruns')
    ...
    def _clean_metrics(self, metrics: dict | None) -> dict:
        """Remove keys com None/NaN e converte valores para float."""
        if not metrics:
            return {}
        cleaned: dict = {}
        for k, v in metrics.items():
            if v is None:
                continue
            # aceita numpy types, strings numéricas, ints, floats
            try:
                val = float(v)
            except Exception:
                continue
    ...
    def logar_melhor_modelo(
        ...
        with mlflow.start_run(
            ...
            mlflow.log_metrics(self._clean_metrics(train_metrics))
            ...
            for caminho in plot_paths.values():
                if caminho and Path(caminho).exists():
                    ...
                    self.logger.info("Artefato %s", caminho)
            ...
            try:
                ...
                artifacts_root = client.list_artifacts(run.info.run_id, path="")
                artifacts_model = client.list_artifacts(run.info.run_id, path="model")

                self.logger.info("Artifacts root: %s", artifacts_root)
                self.logger.info("Artifacts Model: %s", artifacts_model)
            except Exception as e:
                if self.logger:
                    self.logger.warning("Falha ao registrar o modelo: %s", e)
    ...
    def logar_holdout(
        ...
        with mlflow.start_run(run_id=run_id):
            ...
            if holdout_plot_path and Path(holdout_plot_path).exists():
                mlflow.log_artifact(str(holdout_plot_path), artifact_path='model')
                if self.logger:
                    self.logger.info("Holdout logado em %s", holdout_plot_path)
            return True
    ...
    def registrar_modelo(self, run_id: str, registry_name: str) -> None:
        ...
        if self.logger:
            self.logger.info("Model URI: %s", model_uri)        
        try:
