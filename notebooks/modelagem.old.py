...
tracker = MLflowTracker(
    tracking_uri="sqlite:///home/wagner/MLOps/mlflow.db",  # ajuste se necessário
...
mlflow.set_tracking_uri(modeling_cfg["tracking_uri"])
...
# Carrega features (sem target)
X = pq.read_table(features_file).to_pandas()

# Carrega target do processed
...
for model_name, model_cfg in models_cfg.items():
    ...
    with mlflow.start_run(
        ...
        all_results[model_name]['study'] = study
    ...
    if best_meta_alpha is not None:
        ...
        all_results['stacking'] = {
            **agg_stacking,
            ...
            'study': stacking_study   # ← ESSENCIAL
        }
...
# SECAO 8 - Análise do Melhor Modelo

logger.info('-' * 100)
logger.info('SECAO 8: Análise do Melhor Modelo - %s', best_model_name)

if '_instance' in best_result:
    best_model = best_result['_instance']
else:
    best_model=_build_pipeline(
        model_cfg=best_result['model_cfg'],
        model_params=best_params,
        reducer_params=best_result.get('reducer_params', _default_reducer_params()),
        pipe_cfg=pipe_cfg
    )

t0_fit = time.time()
best_model.fit(X_train, y_train)
logger.info('Modelo treinado em %.1fs', time.time() - t0_fit)

y_pred_train = best_model.predict(X_train)
residuals=y_train.values - y_pred_train
train_metrics=_compute_metrics(y_train.values, y_pred_train)
logger.info('Treino - RMSE: %.2f | MAE: %.2f | R2: %.4f', train_metrics['rmse'], train_metrics['mae'], train_metrics['r2'])

fig, ax = plt.subplots(figsize=(10,6))
ax.scatter(y_pred_train, residuals, alpha=0.25, s=6, color='steelblue', edgecolors='none')
ax.axhline(0, color='crimson', linewidth=1.5, linestyle='--', label='Residuo = 0')
ax.set_xlabel('Valores Previstos ($)')
ax.set_ylabel('Residuos ($)')
ax.set_title(f'Residuos vs Valores Ajustados - {best_model_name}')
ax.legend()
plt.tight_layout()
residuals_path = output_dir / 'residuals.png'
fig.savefig(residuals_path, dpi=120, bbox_inches='tight')
plt.close(fig)
logger.info('Plot salvo: %s', residuals_path)
...
ax.set_title(f'Previstos vs Reais - {best_model_name}\nr2 = {train_metrics["r2"]:.4f}')
ax.legend()
plt.tight_layout()
pred_vs_actual_path = output_dir / 'pred_vs_actual.png'
fig.savefig(pred_vs_actual_path, dpi=120, bbox_inches='tight')
plt.close(fig)
logger.info('Plot salvo: %s', pred_vs_actual_path)
...
axes[0].axvline(np.mean(residuals), color='orange', linewidth=1.5,
                linestyle='-.', label=f'Média: {np.mean(residuals):.0f}')
...
fig.suptitle(f'Análise de Erros — {best_model_name}', fontsize=13, fontweight='bold')
plt.tight_layout()
error_dist_path = output_dir / 'error_dist.png'
fig.savefig(error_dist_path, dpi=120, bbox_inches='tight')
plt.close(fig)
logger.info('Plot salvo: %s', error_dist_path)

# Plot 4: Importância de Features
try:
    imp_series = _get_feature_importance(
        best_model,
    ...
    ax.set_title(f'Importância de Features — {best_model_name}')
    ax.set_xlabel('Importância')
    plt.tight_layout()

    feat_imp_path = output_dir / 'feature_importance.png'
    fig.savefig(feat_imp_path, dpi=120, bbox_inches='tight')
    plt.close(fig)
    logger.info('Plot salvo: %s', feat_imp_path)
    logger.info('Top 5 features: %s', top20.head().to_dict())

except Exception as exc:
    logger.warning('Falha ao calcular importância de features: %s', exc)
    feat_imp_path = None

# Plot 5: RMSE por Fold (Robustez)
fold_rmse = [fm['rmse'] for fm in best_result['fold_metrics']]
fold_r2 = [fm['r2'] for fm in best_result['fold_metrics']]
fold_labels = [f'Fold {i+1}' for i in range(len(fold_rmse))]

fig, ax = plt.subplots(figsize=(10, 6))
ax.bar(fold_labels, fold_rmse, color='steelblue', alpha=0.8)
ax.axhline(np.mean(fold_rmse), color='crimson', linestyle='--', linewidth=1.5,
           label=f'Média: {np.mean(fold_rmse):.0f}')
ax.set_ylabel('RMSE')
ax.set_title(f'RMSE por Fold — {best_model_name}')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
rmse_fold_path = output_dir / 'rmse_folds_cv_fold_comparison.png'
fig.savefig(rmse_fold_path, dpi=120, bbox_inches='tight')
plt.close(fig)
logger.info('Plot salvo: %s', rmse_fold_path)


# Learning Curve 1:34:11
is_complex_ensemble = isinstance(best_model, (StackingRegressor, VotingRegressor))

if is_complex_ensemble:
    logger.warning('Learning curve ignored for ensemble (high computation resource needed).')
    ...
    try:
        ...
        # sklearn retorna neg rmse -> inverte sinal
        media_treino = -train_scores.mean(axis=1)
        std_treino   = train_scores.std(axis=1)
        media_val    = -val_scores.mean(axis=1)
        std_val      = val_scores.std(axis=1)

        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(train_sizes_abs, media_treino, 'o-', color='steelblue', label='Treino', linewidth=2)
        ax.fill_between(train_sizes_abs, media_treino - std_treino, media_treino + std_treino, alpha=0.15, color='steelblue')
        ax.plot(train_sizes_abs, media_val, 's-', color='crimson', label='Validação (CV)', linewidth=2)
        ax.fill_between(train_sizes_abs, media_val - std_val, media_val + std_val, alpha=0.15, color='crimson')
        ...
        if logger:
            logger.warning('Learning curve falhou: %s', exc)

# Plota a avaliaçao do Holdout
y_pred_holdout = best_model.predict(X_holdout)
holdout_metrics = _compute_metrics(y_holdout.values, y_pred_holdout)
...
plt.tight_layout()
holdout_evaluation_path = output_dir / 'holdout_evaluation.png'
fig.savefig(holdout_evaluation_path, dpi=120, bbox_inches='tight')
plt.close(fig)
logger.info('Plot salvo: %s', holdout_evaluation_path)


# Histórico de otimizaçao por trial do Optuna
study = all_results[model_name].get('study')
print(f"Study Trials:: {len(study.trials)}")

if study is not None and len(study.trials) >= 1:
    try:
        fig_ax = optuna.visualization.matplotlib.plot_optimization_history(study)
        fig_ax.figure.set_size_inches(10, 5)
        optuna_history = output_dir / 'optuna_history_optimization.png'
        fig.savefig(optuna_history, dpi=120)
        logger.info('   Salvo relatório do histórico de otimizaçao: %s', optuna_history)
    except Exception as exc:
        if logger:
            logger.warning('Plot optuna_history falhou para %s: %s', model_name, exc)
...
if study is not None and len(study.trials) >= 1:
    try:
        ...
        fig_ax.figure.set_size_inches(10, 5)
        optuna_params = output_dir / 'optuna_params_importance.png'
        fig.savefig(optuna_params, dpi=120)
        logger.info('   Salvo relatório Optuna params importance: %s', optuna_params)
    except Exception as exc:
        if logger:
            logger.warning('Plot optuna_params falhou para %s: %s', model_name, exc)


# Ajuste estes nomes/paths se necessário
REGISTERED_MODEL_NAME = "california-housing-best"


# 2) Caminhos de plots/artefatos (se existirem)
plot_paths = {
    #"train_plot": output_dir / "train_diagnostics.png",
    "holdout_plot": output_dir / "holdout_evaluation.png",
}

# 3) Treine/fit o pipeline final no conjunto de treino completo (se ainda não estiver treinado)
#    Observação: se pipeline_tuned já foi treinado durante CV/optuna, pule esta etapa.
try:
    if not hasattr(pipeline_tuned, "fit") or getattr(pipeline_tuned, "fitted_", False) is False:
        pipeline_tuned.fit(X_train, y_train)
except Exception:
    # Se pipeline_tuned já estiver treinado, ignore
    pass

# 4) Loga o melhor modelo e retorna run_id

run_id = tracker.logar_melhor_modelo(
    model_name=REGISTERED_MODEL_NAME,
    model=pipeline_tuned,
    best_params=best_params,
    reducer_params=best_red_params,
    cv_metrics={
        "cv_rmse_mean": agg_tuned["cv_rmse_mean"],
        "cv_rmse_std": agg_tuned["cv_rmse_std"],
        "cv_r2_mean": agg_tuned.get("cv_r2_mean", None),
    },
    # train_metrics={
    #     "rmse": float(train_metrics['rmse']) if 'train_rmse' in globals() else None,
    #     "mae": float(train_metrics['mae']) if 'train_mae' in globals() else None,
    #     "r2": float(train_metrics['r2']) if 'train_r2' in globals() else None,
    #     "mape": float(train_metrics['mape']) if 'train_mape' in globals() else None,
    # },
...
hldout_ok = tracker.logar_holdout(
    run_id=run_id,
    holdout_metrics=holdout_metrics,
    delta_pct=100.0 * (holdout_metrics['rmse'] - agg_tuned['cv_rmse_mean']) / agg_tuned['cv_rmse_mean'],
    holdout_plot_path=plot_paths['holdout_plot']
)

logger.info("Run criado para o melhor modelo: %s", run_id)

# 5) Registrar o modelo no Model Registry (cria a versão)
try:
    if hldout_ok:
        tracker.registrar_modelo(run_id=run_id, registry_name=REGISTERED_MODEL_NAME)
        logger.info("Pedido de registro enviado para o Model Registry: %s", REGISTERED_MODEL_NAME)
except Exception as exc:
    logger.warning("Falha ao registrar modelo automaticamente: %s", exc)

# 6) Mensagem final com instruções rápidas
logger.info("Para confirmar: abra o MLflow UI e verifique Models -> %s", REGISTERED_MODEL_NAME)
logger.info("Ou registre manualmente com: mlflow register-model --model-uri runs:/%s/model --name %s", run_id, REGISTERED_MODEL_NAME)